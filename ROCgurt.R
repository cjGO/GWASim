...
#pcutoff =  c(10^-seq(0,7,.01),seq(0,1,.0005))
...
#fit1 = lm(sample50$list50t ~ poly(sample50$list50f,4,raw=TRUE)) # polynomial regression for each delta p : not used

plot(sample50, xlab = "FPR", ylab = "TPR", pch = 21, bg= 'red', ylim = c(0,1), xlim= c(0,1), cex=.5, main = c('Delta P = ', deltaP))
par(new=TRUE)
plot(sample100, xlab = "FPR", ylab = "TPR", pch = 21, bg= 'blue',ylim = c(0,1), xlim= c(0,1), cex=.5) #type='n' removes points
par(new=TRUE)
plot(sample150, xlab = "FPR", ylab = "TPR", pch = 21, bg= 'green', ylim = c(0,1), xlim= c(0,1), cex=.5)
par(new=TRUE)
plot(sample200, xlab = "FPR", ylab = "TPR", pch = 21, bg= 'orange',ylim = c(0,1), xlim= c(0,1), cex=.5)

legend(.6,.3, cex=.5, c("25 depth","50 depth","75 depth","100 depth"), lty=c(1,1), lwd=c(1,1),col=c("red","blue","green","orange"))
